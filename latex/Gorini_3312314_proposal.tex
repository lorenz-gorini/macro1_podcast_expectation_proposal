...
A standard benchmark in macro is full-information rational expectations (FIRE), where agents behave as if the relevant aggregate state can be directly observed (or costlessly inferred). A growing literature instead emphasizes that agents learn from \emph{noisy, dispersed, and strategically relevant} signals, so that equilibrium expectations can deviate systematically from FIRE even when agents are otherwise rational \citep{AngeletosLian2018,Lorenzoni2009DemandShocks}. Empirically, survey forecasts exhibit predictable forecast errors and infrequent updating patterns, consistent with information frictions such as sticky or noisy information \citep{CoibionGorodnichenko2015,MankiwReis2002StickyInfoVsStickyPrices,Carroll2003MacroExpectationsHouseholdsForecasters}.

In practice, the signals households receive are not drawn from an exogenous Gaussian experiment: they are \emph{produced} by information intermediaries---news, social media, and increasingly long-form audio---operating in attention-driven markets. Competition for attention and audience segmentation can reward one-sided framing and selective emphasis \citep{Mullainathan2005,GentzkowShapiro2010,Shiller2017}. This ``narrative production'' view is especially relevant for macro expectations because the resulting noise can be \emph{correlated within narrative communities}: exposure is clustered, and thus belief distortions may co-move across households inside the same media environment, consistent with the broader evidence on selective exposure and ideological segregation in information consumption \citep{BakshyMessingAdamic2015,GonzalezBailonEtAl2023,MartinYurukoglu2017}.

A second strand of research highlights that deviations from FIRE reflect not only \emph{infrequent} updating, but also \emph{distorted} updating. In particular, forecast errors are often negatively related to prior forecast revisions---a signature consistent with overreaction to recent news \citep{CoibionGorodnichenko2015}. The diagnostic expectations framework formalizes this pattern via a diagnosticity wedge $\theta$, whereby agents overweight recent signals and generate predictable forecast errors from revisions \citep{BordaloGennaioliMaShleifer2020}. More broadly, models of selective memory and cue-triggered retrieval imply that contextual cues and narratives can shift beliefs by changing what comes to mind, even holding objective information fixed \citep{BordaloGennaioliShleifer2020MemoryAttentionChoice}. Related work links belief distortions to how people process statistics and simulate future outcomes, providing microfoundations for state-contingent over- and underreaction \citep{BordaloConlonGennaioliKwonShleifer2026HowPeopleUseStatistics,BordaloBurroCoffmanGennaioliShleifer2025ImaginingFuture}.

These mechanisms also naturally predict \emph{heterogeneity}. SES can shape both exposure to information and the capacity or incentives to update. For example, \citet{difrancesco2024_sticky_information_wealth} documents that households update aggregate expectations infrequently and that wealthier households update more frequently, with quantitative implications for monetary transmission in HANK environments. This motivates our focus on SES as a dimension along which narrative exposure and susceptibility to distorted updating may differ, with downstream effects on household choices relevant for inequality \citep{GarnacheSchmalzSommervollStickyAssetPrices}.
...
\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{images/fed_expectations_by_income.png}
    \caption{Predicted conditional quantiles of household inflation expectations across demographic groups from the New York Fed Survey of Consumer Expectations (SCE): December 2019. There are 24 groups in total: 3 income groups, 2 education groups, 2 homeownership groups, 2 gender coups. Lower-income households consistently report higher inflation expectations than higher-income households. Source: \citet{doh_lee_park_2024_rwp2406}.}
    \label{fig:fed_expectations_by_income}
\end{figure}

\subsection{Podcasts as narrative exposure}
Podcasts provide a tractable setting to measure narrative exposure. They have become a major channel for news and economic commentary: 54\% of U.S. adults listened to a podcast in the past year and 32\% report getting news from podcasts \citep{pew_podcasts_news_fact_sheet_2025}.
Podcasts are a natural proxy for narrative communities that may generate \emph{correlated} noise in households' information sets. Unlike X/Twitter—where usage is limited and discourse is disproportionately produced by a small set of highly active users—podcast listening has broad reach, especially among adults in 30-49 age group, where 62\% listened to podcasts in past year \citep{pew_podcasts_news_fact_sheet_2025}.
Relative to traditional TV news, podcasts disproportionately reach younger and middle-aged cohorts and deliver long-form, repeated narratives that can operate as attention and memory cues \citep{BordaloGennaioliShleifer2020MemoryAttentionChoice}.
Moreover, compared to shorter-form media, podcasts frequently deliver persistent, interpretive storylines (often with ideological slant), making them a natural proxy for sustained narrative environments.

Descriptively, podcasts combine (i) frequent exposure to explicit political framing and (ii) high perceived credibility. In a nationally representative Pew Research Center survey (Dec.\ 5--11, 2022), 54\% of U.S.\ podcast listeners report that the podcasts they listen to include host/guest opinions about government and politics at least sometimes (8\% extremely often, 17\% fairly often, 29\% sometimes). Among listeners who hear political opinions, 47\% say the views mostly line up with their own, 46\% report an even mix, and only 7\% report that the views do not line up. Among listeners who hear news discussed on podcasts, 87\% expect that news to be mostly accurate (vs.\ 11\% mostly inaccurate), and most trust it at least as much as other sources (31\% trust it more; 55\% about the same; 15\% less). Finally, the partisan composition of podcast listeners is relatively balanced (46\% Rep/Lean Rep vs.\ 54\% Dem/Lean Dem), and conditional on being a podcast listener, similar shares of Republicans and Democrats report hearing news discussed on the podcasts they listen to (65\% vs.\ 69\%). \citep{pew_podcasts_news_fact_sheet_2025}

\footnote{These alignment and trust measures are self-reported and may be affected by motivated reasoning (e.g., respondents may perceive congenial narratives as ``objective'' rather than ``aligned''). We treat them as descriptive and complement them with transcript-based measures of narrative slant and diagnosticity in the empirical analysis. See also evidence on partisan bias in reported economic expectations in \citet{miansufi2023}.}

More generally, persuasion research finds that immersive narratives shift beliefs and attitudes, consistent with a mechanism in which sustained framing affects how households interpret macro signals \citep{GreenBrock2000Transportation,BraddockDillard2016MetaAnalytic}. This makes podcasts a complementary setting to the existing evidence on persuasion and selective exposure in cable news and social platforms \citep{DellaVigna2007,MartinYurukoglu2017,BakshyMessingAdamic2015,GonzalezBailonEtAl2023}.

Operationally, podcasts also enable measurement: episode metadata and transcripts can be scraped (e.g., via Listen Notes), allowing construction of show-level narrative polarity/sentiment indices and region-by-SES exposure measures that can be paired with expectations microdata in a shift--share design.

\begin{figure}[htbp]
    \centering
    \begin{minipage}[t]{0.32\linewidth}
        \centering
        \includegraphics[width=\linewidth]{\detokenize{images/Just over half of listeners say the podcasts they listen to include political opinions at least sometimes.png}}
    \end{minipage}\hfill
    \begin{minipage}[t]{0.32\linewidth}
        \centering
        \includegraphics[width=\linewidth]{\detokenize{images/Most listeners who get news on podcasts expect it to be mostly accurate.png}}
    \end{minipage}\hfill
    \begin{minipage}[t]{0.32\linewidth}
        \centering
        \includegraphics[width=\linewidth]{\detokenize{images/No large differences between Republicans and Democrats in podcast listening, hearing news on podcasts.png}}
    \end{minipage}

    \caption{Survey evidence on political content, perceived accuracy, and partisanship in U.S. podcast consumption: (left) frequency of political opinions in podcasts; (middle) expected accuracy of podcast news and relative trust vs. other media; (right) podcast listening and news consumption by political affiliation. Source: \citet{pew_podcasts_news_fact_sheet_2025}.}

    \label{fig:podcast-3panel}
\end{figure}
...
\begin{figure}[h!]\centering
    \begin{tikzpicture}[->,>=stealth, node distance=1.5cm, thick]
        ...
        \node[draw, ellipse, shape=ellipse, left of=belief, xshift=-5cm, yshift=0cm, align=center] (ses) {Household SES\\(income, education)};
        \draw[dashed,->] (ses) -- node[above]{moderates}(belief);
        \draw[dashed,->] (ses) |- (decisions);
        \node[draw, ellipse, shape=ellipse, right of=exposure, xshift=4.5cm, yshift=0cm, align=center] (frag) {Media\\Fragmentation};
...
Starting at the top: \textbf{Narrative content} is produced by various media outlets (podcasters, radio shows, news sites). In our context, consider the content of a set of popular economic or political podcasts. At any given time $t$, each show $j$ might be emphasizing certain themes or narratives. Some might be shouting recession alarms, others may be painting a rosy picture of economic growth, often colored by political ideology or other biases. We can think of each show's content as providing a signal about the state of the economy, but one that may contain \textit{bias or noise}.

For example, a highly polarized show might selectively present facts to fit a narrative (“the economy is crashing due to policy X” or “the economy is booming thanks to policy Y”). In expectation-formation terms, these are \textbf{noisy signals} $s_{j,t}$ that agents observe.
...
Household $i$ updates its expectation of $y^*$ upon seeing $s_{i,t}$. A rational Bayesian (with known signal distributions) would compute a weighted average of prior and new signal. A \textbf{diagnostic updater} as in BGMS, however, uses a distorted weighting: effectively $E_{i}(y^*|s) = E_{i}(y^*|\text{prior}) + \theta \big( s_{i,t} - E_{i}(y^*|\text{prior}) \big)$, where $\theta > 1$ means overreaction (overweighting the new signal relative to Bayesian weight) and $\theta < 1$ means underreaction (underweighting). Here we explicitly include $\theta$ as an individual-specific distortion parameter.
In general, $\theta$ could be a function of the signal itself (Bordalo et al model $\theta$ increasing with the surprise magnitude), but as a first approximation we consider it a personal trait or group trait.

Our hypothesis is that $Z_{r,t}$, the narrative exposure, increases $\theta_{i}$ for individuals in that region/time. Intuitively, being in an echo chamber might hype up the perceived importance of signals (making them more ``representative'' in the BGMS sense). For instance, constant repetition of recession talk makes any negative data seem extremely representative of a coming downturn, so people overweight that data. Formally, one could derive from a psychological model that hearing confirming narratives increases the agent's confidence in the signal (or reduces perceived variance of noise), which leads them to put a higher weight on it. Thus, $Z$ could modulate the weight $\theta$.
...
The main research question driving this proposal is:
\begin{quote}
...
From this, to test the conceptual links, we will estimate parameters that correspond to arrows in Fig.~\ref{fig:dag}:
\begin{itemize}
...
e.g., how much does higher $Z$ (\emph{exposure}) increase $\theta$ (\emph{belief distortion}) on average? How much more does a low-education individual react per unit $Z$ than a high-education one? Does high fragmentation increase the variance of $E_i(y^*)$ controlling for fundamentals?
...
\subsection{Measuring Belief-Updating Distortions \texorpdfstring{$\theta$}{theta}}
...
\begin{itemize}
    \item $\phi > 0$: forecast errors tend to have the same sign as the revision, implying underreaction. This aligns with \textit{information rigidity/stickiness}  models (e.g., sticky/noisy information) and the Coibion--Gorodnichenko forecast-error--revision diagnostic \citep{CoibionGorodnichenko2015,MankiwReis2002StickyInfoVsStickyPrices}. Recent evidence suggests the implied rigidity is heterogeneous by wealth, with wealthier households updating more frequently \citep{difrancesco2024_sticky_information_wealth}.
    \item $\phi < 0$: errors are opposite to revisions. If forecast was revised up, error is likely negative ($F$ too high). This implies overreaction: they over-adjusted to the news. This is what diagnostic expectations predict \citep{BordaloGennaioliMaShleifer2020}.
\end{itemize}
...
\subsection{Exposure Indices \texorpdfstring{$Z^{int}$, $Z^{dir}$, $Z^{frag}$}{Zint, Zdir, Zfrag}}
...
\begin{enumerate}
    \item \textbf{Narrative Intensity ($Z^{\text{int}}_{r,t}$):} This index measures how intensely the population in region $r$ is exposed to partisan or biased narratives at time $t$. Operationally, we define
          $$
    ...
    \item \textbf{Narrative Fragmentation ($Z^{\text{frag}}_{r}$):} This index is more static (or slow-moving). It measures how fragmented or concentrated the media landscape is in region $r$. One way to measure fragmentation is the effective number of popular podcasts. For example, compute a Herfindahl-Hirschman Index (HHI) of podcast audience shares in region $r$:
          ...
          This index does not vary (much) over time in our design, but it will be used in cross-sectional analysis and as an interaction term: does high fragmentation correlate with higher disagreement in expectations, controlling for other factors?
\end{enumerate}
...
\begin{equation}
    \label{eq:main}
    \text{Error}_{i,t+h} = \beta_0 + \beta_1 Z^{\text{int}}_{r(i),t} + \beta_2 \left(Z^{\text{int}}_{r(i),t} \times \mathbf{1}_{i,\text{low-SES}}\right) + \gamma \text{X}_{i,t} + \mu_r + \tau_t + \varepsilon_{i,t+h}.
\end{equation}
...
\begin{equation}
    ...
    \text{Error}_{i,t+h} = \alpha + \phi \text{Revision}_{i,t} + \psi Z^{\text{int}}_{r,t} + \kappa \left(\text{Revision}_{i,t} \times Z^{\text{int}}_{r,t}\right) + \lambda_{r,t} + \epsilon_{i,t}.
\end{equation}
...
$$
    \text{Error}_{i,t+h}=\phi \text{Revision}_{i,t}+ \kappa (\text{Revision}_{i,t} \times Z_{r,t} \times \mathbf{1}_{\text{low-SES}})+\lambda_{r,t}+\text{other interactions}+\epsilon.
$$

In words, with region-time FEs, we identify off differential slopes by SES within the same region and time. For example, we ask: in a given region-month, did low-education respondents' forecast errors vs revisions look systematically different from high-education respondents' in a way that correlates with the region's exposure level? This triple difference (low vs high education, high vs low exposure, before vs after revision) is complex but feasible. We will likely implement it as an interaction IV: use something like $(Z_{r,t} \times \mathbf{1}_{\text{low-SES}})$ as an instrument for the interacted term in the CG regression.

Alternatively, we may aggregate to region-level or region-SES cell level. For instance, compute $\hat{\phi}_{r, \text{low-SES}}$ and $\hat{\phi}_{r, \text{high-SES}}$ from separate CG regressions by group, then regress those on $Z_r$ (with instrumentation). This two-step might be more transparent: "does high exposure correlate with more negative $\hat{\phi}$ (overreaction) for low-SES group?" etc. The challenge is standard errors and some errors-in-variables (we can possibly do a weighted IV on those estimates with known variances).
...
We will test the validity of the instrument in several ways:

\begin{itemize}
    \item Check balance: ensure that pre-treatment characteristics of regions (like average historical expectations or socio-demographics) are not correlated with the instrument (shock exposure).
    \item Placebo tests: apply the same IV strategy to a pre-sample period where we should see no effect (e.g., use base shares from pre-2018 with podcast shocks in 2019-2020 to predict expectation changes in 2015-2016—should be null because 2015-16 expectations couldn't be caused by later podcast content).
    \item Use the methodology of \citet{Adao2019} to test for correlation between shares and shocks. They suggest regressing shares on some function of shocks or vice versa to see if any pattern emerges. Ideally, each shock $N_{j,t}$ should not be disproportionately loading on shares from systematically different regions (that would be like one shock targeting specific region types).
    \item Over-identification: if we treat each major podcast shock as an instrument separately, we could test over-ID (though many shocks likely).
    \item Exogeneity to local economic outcomes: we can check if our instrument predicts local economic variables (like local unemployment changes) when it shouldn't. If we find that, say, a high exposure shock coincides with a local unemployment spike, that suggests instrument may be picking up that local shock effect (bad).
\end{itemize}

In the second stage, we will be careful in interpreting coefficients. Due to the presence of fixed effects and interactions, the effective instrument is $\left(Z^{int}_{r,t} \times \mathbf{1}_{i,\text{low-SES}}\right)$ for the triple diff perhaps. We will implement a suitable IV estimation (possibly a multi-dimensional one where we instrument each endogenous term with a corresponding shift-share).
...
We will discuss ways to address this:
\begin{itemize}
    \item We can attempt to measure the fraction of actual listeners in each region (e.g., from external data like Edison Research which might give fraction of population that listens to podcasts weekly). If we have that, we could scale our coefficients or at least say: if X\% are listeners and we find Y effect on average, then the effect on listeners is roughly Y/X.
    \item In surveys, sometimes they ask how often you get news from source X. If SCE or other data has any question on media usage, we could use that to subset or to do an IV at individual level (like instrument individual's listening by region instrument).
    \item As an extension, we might incorporate a model for partial compliance: treat $Z_{r,t}$ as instrument for an unobserved individual-level variable indicating actual narrative consumption. But without individual media data, that remains abstract.
\end{itemize}
...
Apart from the main causal tests, we will leverage a few supplementary analyses:

\begin{itemize}
    \item \textbf{Directional effects:} Use $Z^{dir}$ to see if, for example, right-leaning narrative exposure predicts a bias in one direction of forecast error (maybe overestimating economic performance under a Democratic president, consistent with partisan pessimism, etc.). This can help confirm that it's indeed narrative content, not just any content, causing biases.
    \item \textbf{Time-series event studies:} Identify specific episodes where a narrative shock occurred (e.g., a popular podcast released a sensational series at a known date) and see if expectations in high-share regions moved differently around that time compared to low-share regions, controlling for national trend. This event study approach can visualize the timing alignment, strengthening causal claims.
    \item \textbf{Robustness to alternate $Z$ definitions:} e.g., define exposure by including talk radio or other media if data allow, or use different base period for shares to ensure results not sensitive.
    \item \textbf{Robustness to including local controls:} We will attempt to include controls for local economic conditions (like region unemployment rate, etc.). In a shift-share IV, including endogenous controls is tricky, but we can check OLS with controls to see if coefficient drops, as a gauge of omitted variables.
    \item \textbf{Dispersion regressions:} as mentioned, test H4 by regressing disagreement on fragmentation or on $Z$.
\end{itemize}
...
For information on datasets, we refer to the Appendix \ref{appendix_datasets}.
...
\paragraph{Confounding by Unobserved Regional Shocks:} Regions might experience unique shocks (economic or otherwise) that coincide with narrative exposure changes. For instance, a hurricane hitting region $r$ could both depress local outlook and also cause more media consumption (people stay home listening to news).
...
\paragraph{Measurement Error in $Z$:} Our constructed exposure indices are noisy proxies of true narrative exposure. This classical measurement error would bias coefficients toward zero (attenuation), making it harder to find effects.

\textit{Mitigation:} The IV actually helps here too, since it's based on the same data but focusing on variation with better signal-to-noise (the aggregated shock). We will improve measurement by smoothing (e.g., using a 3-month moving average of $Z$ if needed) or dimension reduction (maybe use principal component if multiple correlated narrative indices). We'll also do robustness: e.g., define $Z$ using a different base period or excluding some podcasts and see if results hold, to ensure it's not driven by one outlier measurement.

\paragraph{Exogeneity of Shift-Share Instrument:} For the Bartik instrument to be valid, two conditions: \textit{exogeneity of shocks} (the $N_{j,t}$ are unrelated to target outcome except through shares) and/or \textit{exogeneity of shares} (the $\text{Share}_{r,j}^{base}$ are as-good-as-random across $r$ after controlling for fixed effects). In our context, one worry is that the distribution of podcast popularity (shares) is not random—e.g., maybe conservative podcasts are popular in areas that also historically have certain economic volatility. Or shock exogeneity: maybe a particular podcast's content surge is itself a reaction to a localized phenomenon (though less likely as they target national audiences).

\textit{Mitigation:} We will apply tests as recommended by \citet{borusyakhull2025}. For example, we can test for correlation between the share-weighted sums of pre-period characteristics and the shock series. We might also use the procedure of creating pseudo-shocks to test if any structure in shares would create bias. If a particular podcast (shock) is dominant and potentially problematic, we can exclude it and re-run (jackknife exclusion). If results remain, that increases confidence. Furthermore, if many shocks are small, the law of large numbers helps validity \citep{borusyakhull2025}; if one shock is large (like one show contributes a lot), we may show that its pattern still fits exogeneity (maybe through narrative that it wasn't targeting specific regions).

\paragraph{Local Spillovers and SUTVA violations:} If one region's narrative exposure can spill over to another (e.g., people in neighboring region also listen), or if people travel, it could blur treatment. We assume region $r$ exposure mainly affects region $r$. With online media, geography is less tight, but our region definition might approximate communities of interest.

\textit{Mitigation:} Use broad enough regions that cross-listening is minimal (if using state, probably okay; if using city, someone might commute from suburb which is another region in data). Also, if exposure is national but we attribute regionally, that's okay for instrument since national part is in time FE, only differential part matters. We will be cautious interpreting spatial differences.

\paragraph{Multiple Hypothesis / p-hacking concerns:} We have several outcomes (inflation expectations, income expectations, etc.) and multiple indices ($Z^{int}, Z^{dir}$). There is a risk of cherry-picking significant results.
...
This project can be extended in various directions. A few promising ones:
...
\begin{itemize}
    ...
    \item \textbf{Dynamic Effects and Persistence:} We plan to check if the effect of a narrative shock on expectations is persistent or fades as reality unfolds. If narratives cause overshooting, but then people correct (like a boom of pessimism then mean reversion), that's different from permanently anchoring expectations to a biased view. Studying the time dynamics (with panel data tracking same person's expectations after shock dissipates) could be insightful.
\end{itemize}
...
\section*{Appendix 1: Related Literature and Gap}
Our proposal sits at the intersection of (i) empirical ``belief-updating moments'' in macro expectations and (ii) evidence that the information environment is systematically slanted and persuasive. In a benchmark with full-information rational expectations, forecast errors should be orthogonal to forecast revisions. \citet{CoibionGorodnichenko2015} provide a simple regression test and document pervasive predictability, motivating models with information frictions. A distinct behavioral interpretation is \emph{diagnostic expectations}: \citet{BordaloGennaioliMaShleifer2020} show that at the \emph{individual} level, forecast errors covary negatively with forecast revisions across many macro series (overreaction), and structurally estimate a diagnosticity parameter $\theta$ that rationalizes these moments. This evidence disciplines the object we care about---heterogeneity in diagnostic updating---but leaves open a central question: \emph{what features of the information environment generate the correlated, one-sided ``signals'' that trigger diagnostic overreaction outside the model?}

A separate literature shows that biased media is both \emph{consumed selectively} and \emph{causally persuasive}. \citet{MartinYurukoglu2017} estimate sizable persuasion and tastes for like-minded news using quasi-exogenous variation in cable channel positions, providing a clean template for thinking about slanted narratives as a treatment that can shift beliefs and polarization. This is directly relevant for our setting because podcast ecosystems are similarly segmented and long-form, but existing media designs typically study political outcomes rather than \emph{macro belief updating moments} and do not connect narrative exposure to the diagnostic $\theta$ that enters behavioral macro.

Finally, work on higher-order beliefs emphasizes that macro responses to news depend on what people think others know and how signals aggregate. \citet{AngeletosLian2018} show that relaxing common-knowledge assumptions substantially changes the transmission of news about future policies/fundamentals. Our project operationalizes this insight empirically by treating part of households' information as \emph{correlated within narrative clusters}: we measure time-varying podcast narratives and construct DMA-level (Designated Market Area) exposure indices (direction, intensity, fragmentation), then use shift--share variation to estimate ITT effects of the local narrative environment on the overreaction moment and its SES heterogeneity. \textbf{Gap:} behavioral macro has limited evidence disciplining \emph{correlated narrative noise} as a driver of diagnostic updating. \textbf{Contribution:} we introduce measured narrative shocks and plausibly exogenous exposure variation to identify how polarized narratives distort macro expectation updating and how this distortion differs across SES.

\section*{Appendix 2: Datasets} \label{appendix_datasets}
...
\begin{itemize}
    \item \textbf{Listen Notes API / Podcast Index:} Together with Prof. Schwarz, we are already analyzing the Listen Notes comprehensive podcast database to gather metadata on a broad set of podcasts \citep{listennotes_datasets}. This includes:
          \begin{itemize}
...
For expectations, our primary source will be the \textbf{New York Fed Survey of Consumer Expectations (SCE)} microdata. Key features:
\begin{itemize}
    \item It's a monthly rotating panel (each respondent stays for up to 12 months). - It is nationally representative (internet-based sample).
    \item It asks quantitative expectations on several domains: inflation (1-year and 3-year ahead), income growth, spending growth, house price growth, and the perceived probability of things like unemployment rise, etc. It also asks about uncertainty and some other behaviors (e.g., probability of missing a debt payment, plans to buy a car or house, etc.).
    \item It collects demographics: age, gender, education, income, region (they have region identifiers, possibly state or at least census division, maybe even zip code or county in restricted data).
    \item We will need region identifiers to merge with $Z_{r,t}$. We hope to get at least the state or CBSA (core-based statistical area). If only broad region (Northeast, Midwest etc.), that's too coarse. Prior literature using SCE sometimes has state.
    \item SCE sample size: about 1300 per month. At any time, maybe ~100 per census region. Not huge for state-level, but possibly workable at division level or for large states.
    \item We could aggregate to larger regions (like DMA or state groups) if needed.
\end{itemize}

We will use SCE to measure individuals' forecasts and back out revisions and errors:
\begin{itemize}
    \item We have their forecast at time $t$ for, say, inflation over next 12 months. Then a year later we see actual inflation and compute error.
    \item If a respondent doesn't stay a whole year, we can't get their error. But we can look at group average errors (i.e., did the average in region r at time t overshoot relative to actual? But actual is national inflation, which is common to all - so error differences across regions in inflation would solely come from forecast differences).
    \item Actually, for inflation or national variables, the actual is same for everyone. So any difference in error by region is entirely due to differing forecasts. That is fine since forecast differences are what narrative would cause. (We just have to account for the fact that one common actual means any aggregate shock affects all, but differential exposure could cause differential forecast).
    \item For income or household-specific outcomes, the "actual" might differ by individual (someone actually got a raise or not). SCE might ask them later what their income is. If not, we might use self-reported realized or assume actual = actual macro stat (not perfect).
    \item Alternatively, we focus on inflation expectations as our main dependent variable because it's clearly defined and common. And maybe unemployment or growth expectations if available.
\end{itemize}

Other optional data:
\begin{itemize}
    \item \textbf{University of Michigan's Surveys of Consumers (MSC)}: The dataset is older and has long time-series, but that data at micro level is harder to get and has small sample (500/month) and only at broad region maybe. So SCE is better due to richer individual panel.
    \item \textbf{Consumer Expenditure Survey (CES):} If we wanted to see actual spending behavior changes, CES could be used (it's a rotating panel of spending diaries). We could see if high-exposure households cut spending differently. But linking to narratives may be complex. And CES sample by region might not be large.
    \item \textbf{Home Mortgage Disclosure Act (HMDA) data:} gives all mortgage originations. Perhaps use it to see if, in high narrative areas, fewer mortgages are taken when narrative is pessimistic, etc. But that could be many confounds.
    \item \textbf{Nielsen scanner or retail sales data:} Possibly use something like spending by region (if we have something like credit card data by region and time, we could test if consumption responds to narrative shocks).
    \item \textbf{Fed Consumer Credit Panel (Equifax data):} if accessible, could see if credit card balances or delinquencies change by region after narrative shocks.
\end{itemize}
...
Finally, the major limitation is due to merging by region, which requires careful alignment (e.g., Google Trends is by DMA, we have to map SCE respondents to DMA via their zip code or similar), but also makes identification and causal claims at individual level more challenging.
